...
class OwnersDisplayPage extends Page {
...
class  OwnersDisplayPage_Controller extends Page_Controller {
	...
	function PeopleSearchForm() {
		...
		$fields = new FieldSet();
		$fields->push(new TextField('Person','Search for an owner',$Person));
   ...
   function People() {
	 ...
	 // $people = DataObject::get('Person','VisualID = "$color" AND StatusID = "1"'); //Get all the blacks and the ones that are published.  
		...
		$featured = DataObject::get("Person","VisualID = '{$colorID }' AND Featured=TRUE" ); //
		   ...
		   $people = DataObject::get("Person",
									 "VisualID = '{$colorID }' AND StatusID = '{$workflowID}'",
									  ...
									  "{$SQL_start},50"); // 
		$featured = DataObject::get("Person","VisualID = '{$colorID }' AND Featured=TRUE" ); //G
