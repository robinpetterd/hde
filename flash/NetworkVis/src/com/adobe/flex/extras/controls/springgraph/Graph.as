 	...
 	public class Graph extends EventDispatcher
	{
		...
		public static function fromXML(xml: XML, strings: Array): Graph {
			...
			for each (var edge: XML in xml.descendants(edgeName)) {
				...
				if((fromItem != null) && (toItem != null))
					...
					//trace("From " +  fromItem.id + " toItem " + toItem.id); 
					
					//TOD0 - This is where we need to pull the data from the XML 
					try{ 
						//var string:String = "settings: { alpha: 0.5, color: 0, thickness: 10 }"; 
						
						var thesetting:Object = {};  
							
						thesetting.thickness = edge.attribute("thickness"); 
						thesetting.color = edge.attribute("color");
						//thesetting.color = 0xFFFF0000; 
						thesetting.alpha = edge.attribute("alpha"); 
						
						var data:Object = {}; 
						
						data.settings =  thesetting;
					
						} catch (e:Error) {trace("Ah it went wrong")} 
					
					graph.link(fromItem, toItem,data);
		...
		public function link(item1:Item, item2:Item, data:Object = null):void
		{
			...
			if (data == null) 
				...
				//trace("the link has data"); 
				//trace(data); 
				
				try {	
					var settings: Object = data.settings;
					//trace(settings.alpha);
					//trace(settings.thickness);
					//trace(settings.color);
			   } catch (e:Error) { }
						
			
			}
			//trace(trace(ObjectUtil.toString(item1)));

			var friends: Object = _edges[item1.id];
			//trace("friends made");
			friends[item2.id] = data;
			//trace("friends[item2.id] = data;");

			friends = _edges[item2.id];
			//trace("friends = _edges[item2.id]");

			friends[item1.id] = data;
			//trace("friends[item1.id] = data");

			
			edgeArray = null;
			changed();
			//trace("built that one !!!"); 
			} 
			
		 catch (e:Error)
			{
				trace ("error found in the in xml file"); 
				//trace(ObjectUtil.toString(friends));
		...
		public function getLinkData(item1:Item, item2:Item):Object
		{
			...
			if ((friends != null) && friends.hasOwnProperty(item2.id)) {
				//trace("data link was found");
				//trace(friends[item2.id]); 
