			...
			//cursorID = CursorManager.setCursor(_iconHandClosed);
   			...
   			for (var i: int = 1; i < c.length; i++) {
				...
				if(itemView != drawingSurface) {
					...
					var ptScalePoint:Point = new Point(itemView.x + itemView.width/2, itemView.y + itemView.height/2);
				//	call the function and pass in the object to be rotated, the amount to scale X and Y (sx, sy), and the point object we created
					scaleFromCenter(itemView, scale  , scale , ptScalePoint);

				
					//itemView.x = itemView.x + deltaX;
   					//itemView.y = itemView.y + deltaY;
   				} else {
					trace("drawing surface found"); 
				}
   			}
